...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ questions, idealTimeMs, onRestart, config }) => {
  ...
  const handleExportPhoto = async () => {
    if (dashboardRef.current) {
      // Check for dark mode class on html/body to set background color
  ...
  return (
    <div ref={dashboardRef} className="pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700 bg-gray-50 dark:bg-gray-900 p-4 transition-colors duration-200">
      {/* Header Summary */}
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
             <h2 className="text-3xl font-bold text-gray-900 dark:text-white">Session Analytics</h2>
             {config.subject && (
                <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 text-sm font-bold rounded-lg border border-blue-200 dark:border-blue-800">
                  {config.subject}
                </span>
             )}
          </div>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Breakdown of your performance vs. Ideal targets</p>
        </div>
        <div className="flex gap-6 bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm transition-colors duration-200">
          <div className="text-right">
            <p className="text-xs text-gray-400 dark:text-gray-500 uppercase font-semibold">Total Time</p>
            <p className="text-xl font-mono font-bold text-gray-800 dark:text-white">{formatTime(totalTimeSpentMs)}</p>
          </div>
          <div className="w-px bg-gray-200 dark:bg-gray-700"></div>
          <div className="text-right">
            <p className="text-xs text-gray-400 dark:text-gray-500 uppercase font-semibold">Ideal Time</p>
            <p className="text-xl font-mono font-bold text-gray-500 dark:text-gray-400">{formatTime(idealTimeMs)}</p>
          </div>
        </div>
      </div>
      ...
      {/* Comparison Badge */}
      <div className={clsx(
        "mb-8 p-6 rounded-2xl border flex items-center gap-4 transition-colors duration-200",
        isUnderBudget 
        ...
        <div className={clsx(
            "p-3 rounded-full",
            isUnderBudget ? "bg-emerald-100 dark:bg-emerald-900/50" : "bg-amber-100 dark:bg-amber-900/50"
        )}>
           <TrendingUp size={24} />
        </div>
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 transition-colors duration-200">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="font-semibold text-lg text-gray-800 dark:text-white">Time Distribution</h3>
          </div>
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-4 -mt-4 italic">Excluding incomplete questions</p>
          
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={timeData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f3f4f6" className="dark:opacity-20" />
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 text-center text-sm">
             {timeData.map((d) => (
               <div key={d.label} className="p-2 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                  ...
                  <div className="text-[10px] text-gray-400 dark:text-gray-500">{d.percentage.toFixed(0)}%</div>
               </div>
             ))}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 transition-colors duration-200">
          <div className="flex items-center gap-2 mb-6">
            <Target className="text-indigo-500 dark:text-indigo-400" size={20} />
            <h3 className="font-semibold text-lg text-gray-800 dark:text-white">Status Breakdown</h3>
          </div>
                    ...
                    <Tooltip 
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-white dark:bg-gray-700 px-3 py-2 rounded-lg shadow-md border border-gray-100 dark:border-gray-600 text-sm">
                ...
                {statusData.map((d) => (
                    <div key={d.statusLabel} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-600 dark:text-gray-300 font-medium">{d.statusLabel}</span>
                        </div>
                        <div className="text-sm font-bold text-gray-800 dark:text-white">
                            {d.count} <span className="text-xs text-gray-400 dark:text-gray-500 font-normal">({d.percentage.toFixed(0)}%)</span>
                        </div>
                    </div>
                ))}
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-red-100 dark:border-red-900/30 overflow-hidden mb-12 transition-colors duration-200">
        <div className="bg-red-50 dark:bg-red-900/20 p-6 border-b border-red-100 dark:border-red-800/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-red-700 dark:text-red-400 text-sm mt-1">Review questions that took ≥ {config.longQuestionThresholdMin} minutes</p>
          </div>
            ...
            <>
              ...
              {isFinalized && (
                <div className="mb-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-top-4">
                   <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl border border-gray-200 dark:border-gray-600">
                      <h4 className="font-bold text-gray-700 dark:text-gray-300 mb-2 text-sm uppercase">Hard / Lengthy %</h4>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 font-semibold mt-1">({metrics.hardPercentage.toFixed(2)}%)</p>
                   </div>
                   <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl border border-gray-200 dark:border-gray-600">
                      <h4 className="font-bold text-gray-700 dark:text-gray-300 mb-2 text-sm uppercase">Accuracy in Hard</h4>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 font-semibold mt-1">({metrics.hardAccuracy.toFixed(2)}%)</p>
                   </div>
                   <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl border border-gray-200 dark:border-gray-600">
                      <h4 className="font-bold text-gray-700 dark:text-gray-300 mb-2 text-sm uppercase">Long Skipped %</h4>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 font-semibold mt-1">({metrics.skippedPercentage.toFixed(2)}%)</p>
                   </div>
                   <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl border border-gray-200 dark:border-gray-600">
                      <h4 className="font-bold text-gray-700 dark:text-gray-300 mb-2 text-sm uppercase">Total Accuracy (Ans)</h4>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 font-semibold mt-1">({metrics.totalAccuracy.toFixed(2)}%)</p>
                   </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 gap-4">
                {longQuestions.map(q => {
                   ...
                   return (
                    <div key={q.id} className="flex flex-col sm:flex-row sm:items-center justify-between bg-gray-50 dark:bg-gray-700 p-4 rounded-xl border border-gray-100 dark:border-gray-600 gap-4 transition-colors duration-200">
                      <div className="flex items-center gap-4">
                        <div className={clsx(
                          "w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg",
                          isSkipped ? "bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300" : "bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300"
                        )}>
                          {q.id}
                        </div>
                        <div>
                          <div className="font-mono font-bold text-xl text-gray-800 dark:text-white">
                        ...
                        {!isSkipped && (
                          <button
                            ...
                            className={clsx(
                              ...
                              review.isCorrect 
                                ? "bg-green-100 dark:bg-green-900/40 border-green-200 dark:border-green-800 text-green-800 dark:text-green-300" 
                                : "bg-white dark:bg-gray-600 border-gray-200 dark:border-gray-500 text-gray-500 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-500",
        ...
        <div className="bg-gray-50 dark:bg-gray-700/30 p-6 border-t border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row justify-end gap-4 transition-colors duration-200">
           <button
             onClick={handleExportCSV}
             className="flex items-center justify-center gap-2 px-6 py-3 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors font-medium shadow-sm hover:shadow-md active:scale-95"
           >
             <Download size={18} /> Export CSV
           </button>
           <button
             onClick={handleExportPhoto}
             className="flex items-center justify-center gap-2 px-6 py-3 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors font-medium shadow-sm hover:shadow-md active:scale-95"
           >
      ...
      <div className="flex justify-center pt-8 border-t border-gray-200 dark:border-gray-700">
        <button
          onClick={onRestart}
          className="flex items-center gap-2 px-8 py-3 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 rounded-lg hover:bg-black dark:hover:bg-white/90 transition-colors font-medium shadow-lg hover:shadow-xl transform active:scale-95"
        >
